{{--
    Vista individual de dimensión — layout reutilizable
...
<x-layouts.app title="{{ $title }} | Dimensiones | Parroquia Cristo Resucitado" description="{{ Str::limit(strip_tags($introText), 160) }}">

    {{-- ═══════════════════════════════════════════════════════
         1. HERO
    ═══════════════════════════════════════════════════════ --}}
    <section class="relative w-full h-[420px] sm:h-[480px] flex items-center justify-center overflow-hidden">
        <div class="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat" style="background-image: url('{{ $heroImage }}');">
            <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-black/30"></div>
        </div>
        <div class="relative z-10 text-center px-4">
            <span class="inline-block px-4 py-1.5 mb-5 text-xs font-bold tracking-widest text-white uppercase bg-primary/80 rounded-full">Dimensión Parroquial</span>
            ...
            <div class="flex flex-col lg:flex-row gap-12 lg:gap-16 items-center">
                ...
                <div class="lg:w-7/12">
                    <div class="bg-white dark:bg-[#211c11] rounded-2xl p-8 sm:p-10 border border-gray-100 dark:border-gray-800 shadow-sm relative">
                        <div class="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-primary to-secondary rounded-t-2xl"></div>
                        <span class="material-symbols-outlined text-primary/30 text-6xl mb-4 block">format_quote</span>
                        <blockquote class="text-xl sm:text-2xl font-medium text-text-dark dark:text-white leading-relaxed italic mb-6">
                            "{{ $quote }}"
                        </blockquote>
                        <cite class="text-primary font-bold text-sm not-italic tracking-wide">{{ $quoteSource }}</cite>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-20 bg-gray-50 dark:bg-[#1a160e]">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-10">
                <span class="text-primary font-bold tracking-wide uppercase text-sm mb-2 block">Nuestra Labor</span>
        ...
        <div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-{{ min(count($opciones), 4) }} gap-8">
                ...
                <div class="group bg-white dark:bg-[#211c11] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden text-center p-8">
                    <div class="w-16 h-16 rounded-2xl bg-{{ $op['color'] ?? 'primary' }}/10 flex items-center justify-center text-{{ $op['color'] ?? 'primary' }} mb-6 mx-auto group-hover:scale-110 transition-transform">
